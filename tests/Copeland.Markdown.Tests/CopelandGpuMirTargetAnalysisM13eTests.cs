...
public sealed class CopelandGpuMirTargetAnalysisM13eTests
{
    ...
    {
        Assert.True(File.Exists(Path.Combine(GetRepositoryRoot(), "docs", "Aurelian", "aurelian-sdslv-lane-audit-m13e.md")));
        Assert.True(File.Exists(Path.Combine(GetRepositoryRoot(), "docs", "Copeland", "copeland-gpu-mir-target-analysis-m13e.md")));
    }
