...
public sealed class AurelianVisibleTriangleTopologyM13gTests
{
    ...
    public void M13gDocs_Exist()
    {
        ...
        Assert.True(File.Exists(Path.Combine(root, "docs", "Aurelian", "aurelian-visible-triangle-topology-audit-m13g.md")));
        Assert.True(File.Exists(Path.Combine(root, "docs", "Copeland", "vd-mir-visible-triangle-proof-boundary-m13g.md")));
    }

    [Fact]
    public void AurelianVisibleTriangleSample_ProjectExists()
    {
        Assert.True(File.Exists(Path.Combine(GetRepositoryRoot(), "samples", "Aurelian.VisibleTriangle", "Aurelian.VisibleTriangle.csproj")));
    }

    [Fact]
    public void AurelianVisibleTriangleSample_IsIncludedInAurelianSolution()
    {
        string text = File.ReadAllText(Path.Combine(GetRepositoryRoot(), "Aurelian.slnx"));
        Assert.Contains("samples/Aurelian.VisibleTriangle/Aurelian.VisibleTriangle.csproj", text, StringComparison.Ordinal);
    }
    ...
    public void M13g_DoesNotAddVisibleTriangleToCopelandSolution()
    {
        ...
        string slow = File.ReadAllText(Path.Combine(root, "Copeland.Slow.slnx"));
