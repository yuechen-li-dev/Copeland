...
public sealed class AurelianPresenterScreenStackM14cTests
{
    ...
    public void M14cDocs_Exist()
    {
        ...
        Assert.True(File.Exists(Path.Combine(root, "docs", "Aurelian", "aurelian-presenter-screen-stack-m14c.md")));
    ...
    {
        string doc = File.ReadAllText(Path.Combine(GetRepositoryRoot(), "docs", "Aurelian", "aurelian-presenter-screen-stack-m14c.md"));
    ...
    {
        string readme = File.ReadAllText(Path.Combine(GetRepositoryRoot(), "samples", "Aurelian.VisibleTriangle", "README.md"));
