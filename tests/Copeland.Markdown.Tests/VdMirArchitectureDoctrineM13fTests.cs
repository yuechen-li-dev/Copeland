...
public sealed class VdMirArchitectureDoctrineM13fTests
{
    ...
    {
        Assert.True(File.Exists(Path.Combine(GetRepositoryRoot(), "docs", "Copeland", "vd-mir-architecture-doctrine-m13f.md")));
    }
    ...
    {
        string text = File.ReadAllText(Path.Combine(GetRepositoryRoot(), "docs", "Copeland", "vd-mir-architecture-doctrine-m13f.md"));
    ...
    {
        Assert.True(Directory.Exists(Path.Combine(GetRepositoryRoot(), "samples", "Aurelian.VisibleTriangle")));
