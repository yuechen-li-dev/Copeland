...
public sealed class CopelandWorkshopArchitectureM13dTests
{
    ...
    {
        Assert.True(File.Exists(Path.Combine(GetRepositoryRoot(), "docs", "Copeland", "copeland-compiler-workshop-architecture-m13d.md")));
        Assert.True(File.Exists(Path.Combine(GetRepositoryRoot(), "docs", "Copeland", "copeland-compiler-lane-taxonomy-m13d.md")));
        Assert.True(File.Exists(Path.Combine(GetRepositoryRoot(), "docs", "Copeland", "README.md")));
        Assert.True(File.Exists(Path.Combine(GetRepositoryRoot(), "docs", "Copeland", "copeland-roadmap.md")));
    }

    [Fact]
    public void CopelandWorkshopDocs_StateNoUniversalIrMandate()
    {
        string text = File.ReadAllText(Path.Combine(GetRepositoryRoot(), "docs", "Copeland", "copeland-compiler-workshop-architecture-m13d.md"));
    ...
    {
        Assert.True(Directory.Exists(Path.Combine(GetRepositoryRoot(), "src", "Aurelian.Shaders")));
    }
