...
public sealed class OblivionWorkspacePersistenceM11dTests
{
    ...
    public void M11d_DoesNotRunFactOrTheoryCards()
    {
        ...
        Assert.DoesNotContain("Xunit.Sdk", string.Join(Environment.NewLine, GetSourceFiles("samples", "Machina.UI", "Machina.Presenter.Sample").Select(File.ReadAllText)), StringComparison.Ordinal);
    }
    ...
    {
        return Path.Combine(GetRepositoryRoot(), "samples", "Machina.UI", "Machina.Presenter.Sample", "OblivionSampleWorkspace", "cards", fileName);
    }
