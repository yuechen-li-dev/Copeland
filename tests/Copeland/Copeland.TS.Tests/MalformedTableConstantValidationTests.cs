...
public sealed class MalformedTableConstantValidationTests
{
    ...
    public static IEnumerable<object[]> Cases()
    {
        ...
        yield return Case("incomplete record", Table(Record([]), RecordType), "does not provide every record field");
    ...
    public void Shared_validator_rejects_malformed_closed_table_constants(string _, MirProgram program, string expectedMessage)
    {
        IReadOnlyList<MirValidationDiagnostic> diagnostics = MirValidator.Validate(program);

        Assert.Contains(diagnostics, diagnostic => diagnostic.Message.Contains(expectedMessage, StringComparison.Ordinal));
    }
