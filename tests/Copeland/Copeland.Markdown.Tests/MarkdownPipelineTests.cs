...
public sealed class MarkdownPipelineTests
{
    ...
    [Fact]
    public void MarkdownToDocumentMir_LowersOrderedList()
    {
        DocumentMir mir = MarkdownCompiler.Compile("1. one\n2. two").Mir;

        ListMir list = Assert.IsType<ListMir>(Assert.Single(mir.Blocks));
        Assert.Equal(DocumentListKind.Ordered, list.Kind);
        Assert.Equal(2, list.Items.Count);
    }
    ...
    {
        string source = File.ReadAllText(GetRepoFile(@"docs\Machina.UI\history\machina-oblivion-phase-closeout-m11g.md"));
    ...
    public void MarkdownCorpus_OblivionBodyFiles_DoNotCrash()
    {
        string[] files =
        [
            @"src\Oblivion\Oblivion.App\OblivionSampleWorkspace\body\oblivion-substrate-status.md",
            @"src\Oblivion\Oblivion.App\OblivionSampleWorkspace\body\markdown-first-roadmap.md",
            @"src\Oblivion\Oblivion.App\OblivionSampleWorkspace\body\markdown-readiness-audit.md",
            @"src\Oblivion\Oblivion.App\OblivionSampleWorkspace\body\selected-doc-dogfood.md",
            @"src\Oblivion\Oblivion.App\OblivionSampleWorkspace\body\markdown-diagnostics-sample.md",
            @"src\Oblivion\Oblivion.App\OblivionSampleWorkspace\body\execution-deferred.md",
            @"src\Oblivion\Oblivion.App\OblivionSampleWorkspace\body\visionary-future.md",
        ];
    ...
    {
        string source = File.ReadAllText(GetRepoFile(@"src\Oblivion\Oblivion.App\OblivionSampleWorkspace\body\markdown-diagnostics-sample.md"));
        ...
        Assert.Contains(MarkdownDiagnosticIds.UnclosedCodeFence, first, StringComparison.Ordinal);
    }

    [Fact]
    public void OblivionMarkdownDogfood_LoadsSelectedDocMarkdown()
    {
        string source = File.ReadAllText(GetRepoFile(@"src\Oblivion\Oblivion.App\OblivionSampleWorkspace\body\selected-doc-dogfood.md"));

        MarkdownCompilation compilation = MarkdownCompiler.Compile(source);

        Assert.NotEmpty(compilation.Mir.Blocks);
        Assert.Contains(compilation.Mir.Blocks, static block => block is HeadingMir);
    }
    ...
    public void M12a_DoesNotReferenceMarkdigOrCommonMark()
    {
        ...
        IEnumerable<string> markdownFiles = Directory
            .EnumerateFiles(Path.Combine(repoRoot, "src", "Copeland", "Copeland.Markdown"), "*.*", SearchOption.AllDirectories)
            .Concat(Directory.EnumerateFiles(Path.Combine(repoRoot, "src", "Copeland", "Copeland.Cli"), "*.*", SearchOption.TopDirectoryOnly));
