...
public static class OblivionWorkbenchCatalog
{
        ...
        OblivionCardEffectState? effectState = null)
    {
        ...
        OblivionActionOutcome? outcome = application.Invoke(
            ...
            pageId,
            actionId,
            new OblivionApplicationState(effectState ?? OblivionCardEffectState.Empty));
        return outcome is null
            ? null
            : new OblivionCardEffectOutcome(outcome.Request, outcome.Result);
    }
        ...
        PresenterProofOptions? proofOptions = null)
    {
        ...
        IReadOnlyList<OblivionCard> cards = CreateAllCards(proofOptions)
            ...
            .ToArray();
        IReadOnlyList<OblivionBuiltCard> builtCards = cards
            .Select(card => CardHandlers.BuildCard(card, card.PageId?.Value, card.WorkspaceId?.Value, OblivionCardEffectState.Empty))
            .OrderBy(card => card.SourceCard.Id.Value, StringComparer.Ordinal)
            .ToArray();
