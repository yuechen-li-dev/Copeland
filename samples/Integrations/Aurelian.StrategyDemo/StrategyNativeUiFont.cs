using System.Security.Cryptography;
using System.Text;
using Aurelian.Machina;
using Aurelian.Machina.Graphics;
using Machina.Core.Measurement;
using Machina.Core.Styling;
using Machina.Fonts;
using Machina.Fonts.Generation;
using Machina.Fonts.Generation.MsdfSharp;
using Machina.Fonts.Generation.Typography;
using Machina.Fonts.ReferenceRendering;
using Machina.Presentation;
using FontRgba = Machina.Fonts.ReferenceRendering.Rgba32;

namespace Aurelian.StrategyDemo;

public sealed class StrategyNativeUiFont : ITextMeasurer
{
    public const int MinimumFieldDimension = 64;

    public const double FieldPixelRange = 8;

    private const string SupportedCharacters =
        " !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~";

    private readonly IReadOnlyDictionary<TextSize, StrategyFontAtlas> atlases;

    private StrategyNativeUiFont(IReadOnlyDictionary<TextSize, StrategyFontAtlas> atlases)
    {
        this.atlases = atlases;
    }

    public IReadOnlyCollection<AurelianMsdfAtlasResource> Resources =>
        atlases.Values.Select(static atlas => atlas.Resource).ToArray();

    public static StrategyNativeUiFont Create(string assetDirectory)
    {
        string serif = Path.Combine(assetDirectory, "CrimsonText-Regular.ttf");
        string mono = Path.Combine(assetDirectory, "SpaceMono-Regular.ttf");
        if (!File.Exists(serif) || !File.Exists(mono))
        {
            throw new FileNotFoundException("Mossward's native UI fonts are missing.");
        }

        return new StrategyNativeUiFont(new Dictionary<TextSize, StrategyFontAtlas>
        {
            [TextSize.Sm] = BuildAtlasAsync(mono, "strategy-mono", 12).GetAwaiter().GetResult(),
            [TextSize.Md] = BuildAtlasAsync(serif, "strategy-serif", 22).GetAwaiter().GetResult(),
            [TextSize.H1] = BuildAtlasAsync(serif, "strategy-serif", 30).GetAwaiter().GetResult(),
        });
    }

    public IntrinsicSize MeasureText(string value, TextStyle style)
    {
        StrategyFontAtlas atlas = Get(style.Size);
        DistanceFieldTextLayoutResult layout = Layout(atlas, value, 0, atlas.Size * 0.8);
        return new IntrinsicSize(Math.Ceiling(layout.Width + 2), Math.Ceiling(atlas.Size * 1.25 + 2));
    }

    public PositionedTextOperation Qualify(PositionedTextOperation operation)
    {
        StrategyFontAtlas atlas = Get(operation.Style.Size);
        ValidateCharacters(operation);
        DistanceFieldTextLayoutResult initial = Layout(atlas, operation.Text, 0, 0);
        double x = operation.Style.AlignX switch
        {
            TextAlignX.Center => (operation.Rect.Width - initial.Width) / 2,
            TextAlignX.Right => operation.Rect.Width - initial.Width,
            _ => 0,
        };
        double baseline = operation.Style.AlignY switch
        {
            TextAlignY.Center => ((operation.Rect.Height - atlas.Size) / 2) + (atlas.Size * 0.8),
            TextAlignY.Bottom => operation.Rect.Height - (atlas.Size * 0.2),
            _ => atlas.Size * 0.8,
        };
        DistanceFieldTextLayoutResult positioned = Layout(atlas, operation.Text, x, baseline);
        return new PositionedTextOperation(
            operation.SourceId,
            operation.Rect,
            operation.Text,
            operation.Style,
            operation.Color,
            new MachinaTextPresentationPrimitive(
                positioned.GlyphRun,
                atlas.Resource.Identity,
                MachinaTextRenderingMode.Msdf));
    }

    public AurelianMsdfAtlasResource ResourceFor(PositionedTextOperation operation)
    {
        MachinaFontAtlasId identity = operation.Primitive?.AtlasIdentity
            ?? throw new InvalidOperationException($"Text operation '{operation.SourceId}' is not native-qualified.");
        return atlases.Values.Single(atlas => atlas.Resource.Identity == identity).Resource;
    }

    private StrategyFontAtlas Get(TextSize size)
    {
        if (!atlases.TryGetValue(size, out StrategyFontAtlas? atlas))
        {
            throw new NotSupportedException($"Mossward does not support native text size '{size}'.");
        }
        return atlas;
    }

    private static void ValidateCharacters(PositionedTextOperation operation)
    {
        foreach (Rune rune in operation.Text.EnumerateRunes())
        {
            if (!Rune.IsWhiteSpace(rune) && !SupportedCharacters.Contains(rune.ToString(), StringComparison.Ordinal))
            {
                throw new InvalidOperationException(
                    $"Mossward native UI text '{operation.SourceId}' contains unsupported glyph U+{rune.Value:X4}.");
            }
        }
    }

    private static DistanceFieldTextLayoutResult Layout(StrategyFontAtlas atlas, string text, double x, double baseline)
    {
        DistanceFieldTextRun run = DistanceFieldTextRun.Create(
            text,
            atlas.Face,
            atlas.Size,
            MachinaFontWeight.Regular,
            MachinaFontSlant.Upright);
        return DistanceFieldTextLayout.Layout(
            run,
            atlas.Metrics,
            new DistanceFieldTextRenderOptions(
                1280,
                800,
                atlas.Face,
                atlas.Size,
                MachinaFontWeight.Regular,
                MachinaFontSlant.Upright,
                DistanceFieldKind.Msdf,
                NextPowerOfTwo(Math.Max(MinimumFieldDimension, atlas.Size)),
                NextPowerOfTwo(Math.Max(MinimumFieldDimension, atlas.Size)),
                FieldPixelRange,
                FontRgba.White,
                new FontRgba(0, 0, 0, 0),
                x,
                baseline,
                PageWidth: 1024,
                PageHeight: 1024,
                PagePadding: 2));
    }

    private static async Task<StrategyFontAtlas> BuildAtlasAsync(string path, string id, int size)
    {
        FontFaceId face = new(id);
        var source = new TypographyGlyphOutlineSource(new Dictionary<FontFaceId, TypographyFontFaceSource>
        {
            [face] = new(face, path, 0),
        });
        var pipeline = new GlyphGenerationPipeline(source, new MsdfSharpDistanceFieldGenerator());
        int dimension = NextPowerOfTwo(Math.Max(MinimumFieldDimension, size));
        var settings = new MsdfGenerationSettings(
            DistanceFieldKind.Msdf,
            dimension,
            dimension,
            FieldPixelRange,
            1,
            "simple",
            2);
        var outlineOptions = new GlyphOutlineLoadOptions(size, 0, GlyphHintingMode.None, normalizeToEm: true);
        GlyphKey[] keys = DistanceFieldTextRun.Create(
                SupportedCharacters,
                face,
                size,
                MachinaFontWeight.Regular,
                MachinaFontSlant.Upright)
            .GlyphKeys
            .Distinct()
            .OrderBy(static key => key.Codepoint)
            .ToArray();
        List<GeneratedGlyphDistanceField> fields = [];
        Dictionary<GlyphKey, GlyphMetrics> metrics = [];
        foreach (GlyphKey key in keys)
        {
            GlyphGenerationResult result = await pipeline.GenerateAsync(key, outlineOptions, settings);
            if (result.Metrics is not null)
            {
                metrics[key] = result.Metrics;
            }
            if (!Rune.IsWhiteSpace(new Rune(key.Codepoint)))
            {
                if (!result.Success || result.DistanceField is null)
                {
                    throw new InvalidOperationException($"MSDF generation failed for U+{key.Codepoint:X4}.");
                }
                fields.Add(result.DistanceField);
            }
        }

        GeneratedFieldAtlasPackResult packed = new GeneratedFieldAtlasPacker().Pack(
            fields,
            new GeneratedFieldAtlasPackOptions(1024, 1024, 2, $"mossward-ui-{id}-{size}"));
        if (!packed.Success)
        {
            throw new InvalidOperationException(
                "Mossward MSDF atlas packing failed: " + string.Join("; ", packed.Diagnostics.Select(item => item.Message)));
        }
        Dictionary<int, byte[]> pages = packed.Pages.ToDictionary(static page => page.Index, EncodeRgba8);
        byte[] identityBytes = pages.OrderBy(static item => item.Key).SelectMany(static item => item.Value).ToArray();
        string hash = Convert.ToHexString(SHA256.HashData(identityBytes)).ToLowerInvariant();
        var identity = new MachinaFontAtlasId($"mossward-{id}-{size}-sha256-{hash}");
        return new StrategyFontAtlas(
            face,
            size,
            metrics,
            new AurelianMsdfAtlasResource(identity, packed.Snapshot, pages, AurelianMsdfAtlasRowOrder.TopToBottom));
    }

    private static byte[] EncodeRgba8(GeneratedFieldAtlasPage page)
    {
        byte[] result = new byte[checked(page.Width * page.Height * 4)];
        for (int pixel = 0; pixel < page.Width * page.Height; pixel++)
        {
            int source = pixel * 3;
            int target = pixel * 4;
            result[target] = ToByte(page.Data[source]);
            result[target + 1] = ToByte(page.Data[source + 1]);
            result[target + 2] = ToByte(page.Data[source + 2]);
            result[target + 3] = 255;
        }
        return result;
    }

    private static byte ToByte(float value) =>
        (byte)Math.Round(Math.Clamp(value, 0, 1) * 255, MidpointRounding.AwayFromZero);

    private static int NextPowerOfTwo(int value)
    {
        int result = 1;
        while (result < value)
        {
            result *= 2;
        }
        return result;
    }

    private sealed record StrategyFontAtlas(
        FontFaceId Face,
        int Size,
        IReadOnlyDictionary<GlyphKey, GlyphMetrics> Metrics,
        AurelianMsdfAtlasResource Resource);
}
