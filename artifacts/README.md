...
- `artifacts/m10a/` is the historical presenter navigation shell export directory.
...
- `artifacts/m18d/` is the Oblivion host-adapter elimination manifest and canonical 14-scenario playback proof directory. It records the closed M18 extraction arc and typed action/effect boundary.
...
- Historical M14a VD-MIR manifests and proof artifacts are deterministic text/JSON/HLSL/SPIR-V-hex files and may remain checked in as chronology and deferred-reviewer context.
...
- Current M17f layout-authoring closeout artifacts are deterministic text/JSON reports. They may be checked in when they document that the stack/grid authoring parity arc is closed, record the current stack/grid baseline, classify remaining deferred parity pressure, preserve the no-runtime-change boundary, and recommend the next implementation direction without claiming new primitive work.
...
Current M9i closeout commands:

```powershell
dotnet test Copeland.slnx
dotnet build Copeland.slnx --no-restore
.\tools\Export-MachinaFontDiagnostics.ps1 -OutputDir artifacts\m9i-font-diagnostics -Preset cad-debug -TextBackend DirectOutlineStatic -GridStep 8 -ShowUnitLabels -ShowBounds -Clean
...
Current M10b presenter interaction commands:
...
Current M10c presenter organization commands:
...
Current M10d presenter stabilization commands:
...
Current M12b Oblivion Markdown body commands:
...
Current M12d Oblivion existing-docs dogfood commands:
...
## Current M10b presenter interaction outputs
...
These remain local sample proof artifacts only. M10b adds sample-level sidebar/tab/wheel interaction wiring through an Avalonia input adapter without changing production renderer behavior, shared layout semantics, or the closed M9 font-phase policy.

## Current M10c presenter organization outputs
...
These remain local sample proof artifacts only. M10c makes the navigation shell the canonical presenter sample surface, preserves the old M1e card as a `Legacy` page, keeps M9 font work closed, adds no new component families, and does not change production renderer/core/layout behavior.

## Current M10d presenter stabilization outputs
