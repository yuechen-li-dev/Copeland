...
There was no Copeland `try`/`except` implementation in this milestone. CTS-M6b/M6c later implement the typed lexical Result-handler design from [CTS-M6a](../language/copeland-ts-try-except-design-cts-m6a.md): it does not catch unwrap panic, and test-host interception of the private throw is not source-language recovery. [CTS-M6d](copeland-ts-typed-fallibility-closeout-cts-m6d.md) ratifies that boundary.
