...
JTF-M6c should remain the previously deferred Copeland-specific work: define and implement the reserved Cope Test source dialect/parser-verifier path without treating existing `.cope` expected-MIR artifacts as a shared source format. It should not reopen source-contract extraction unless the prerequisite evidence exists.
