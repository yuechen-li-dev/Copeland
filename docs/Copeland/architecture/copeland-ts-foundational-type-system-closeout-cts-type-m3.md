...
Independent union work is now tracked by [CTS-UNION-M0b](copeland-ts-nominal-union-sugar-cts-union-m0b.md): declaration-only `type Name = Record | Record` canonicalizes to payload enums without broadening `TypeSyntax`. [CTS-CALL-M0a](../language/copeland-ts-callables-and-explicit-capture-design-cts-call-m0a.md) now owns function types, noncapturing lambdas, first-class callables, and explicit capture; `CTS-TSXML` owns TS-XML; `CTS-STATIC` owns bounded compile-time execution; CLR/.NET interop owns metadata import. Generic nominal types require separate evidence and approval.
