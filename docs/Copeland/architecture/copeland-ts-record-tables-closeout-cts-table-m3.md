...
JSON, `toJSON`, `fromJSON`, parsing, stringification, codecs, serialization, and host interop are **unimplemented**. In particular, `JSON.stringify` of a private JavaScript carrier is neither canonical nor supported. [CTS-TSON-M0a](../language/copeland-ts-tson-design-cts-tson-m0a.md) superseded the old direct CTS-TABLE-JSON routing, and [CTS-TSON-TABLE-M0a](../language/copeland-ts-tson-tables-design-cts-tson-table-m0a.md) now selects the dedicated nominal, table-root-only TSON representation. Its future ladder excludes Result-valued columns initially and routes any later JSON through policy-directed TSON lowering. Builders, mutation, row construction, queries, row-oriented storage, equality/hashing, keys, metadata/iteration, relational/dataframe operations, and all alternate formats remain excluded.
