...
JSON, `toJSON`, `fromJSON`, parsing, stringification, codecs, serialization, and host interop are **unimplemented**. In particular, `JSON.stringify` of a private JavaScript carrier is neither canonical nor supported. [CTS-TSON-M0a](../language/copeland-ts-tson-design-cts-tson-m0a.md) supersedes the old direct CTS-TABLE-JSON routing; completed [CTS-TSON-ARRAY-M0b](copeland-ts-tson-arrays-and-assets-cts-tson-array-m0b.md) establishes typed homogeneous arrays only as its prerequisite. A future table compatibility ladder must still approve table/Result TSON laws, translate logical table values through TSON, and only then lower to or decode from JSON. Builders, mutation, row construction, queries, row-oriented storage, equality/hashing, keys, metadata/iteration, relational/dataframe operations, and all alternate formats remain excluded.
