...
The runtime artifact boundary is unchanged. M14a writes proof artifacts only under `artifacts/m14a` and leaves `samples/Aurelian/Aurelian.VisibleTriangle` on its existing checked-in artifact set.
