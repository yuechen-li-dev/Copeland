...
- `samples/Aurelian/Aurelian.VisibleTriangle` is not wired to `VD-MIR`
...
M13f does not implement M0. M14a later lands the minimal smoke-triangle compiler slice in `src/Aurelian/Aurelian.Shaders/Language/VdMir` without promoting that code into Copeland packages yet.
...
`samples/Aurelian/Aurelian.VisibleTriangle` now exists in the repository and is acknowledged as an important future proof target.
...
- M13g later performs the topology and proof-boundary audit for `samples/Aurelian/Aurelian.VisibleTriangle` without changing the sample's current shader/runtime/render path and without adding any `VD-MIR` implementation.
