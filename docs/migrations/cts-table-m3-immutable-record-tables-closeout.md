...
JSON and every serializer/codec/host interop surface remain unimplemented. [CTS-TSON-M0a](../Copeland/language/copeland-ts-tson-design-cts-tson-m0a.md) supersedes direct table-to-JSON implementation. [CTS-TSON-ARRAY-M0a](../Copeland/language/copeland-ts-tson-arrays-design-cts-tson-array-m0a.md) makes typed homogeneous arrays one prerequisite only; a separately approved table/Result TSON extension must still precede any JSON compatibility lowering, and private JavaScript carriers are neither TSON nor JSON.
