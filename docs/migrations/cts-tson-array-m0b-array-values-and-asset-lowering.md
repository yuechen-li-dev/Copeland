...
The new corpus hashes are recorded in the [ARRAY-M0b architecture record](../Copeland/architecture/copeland-ts-tson-arrays-and-assets-cts-tson-array-m0b.md#completion-evidence). Existing TSON and non-TSON corpus artifacts are unchanged. Runtime TSON array encoding, root arrays, JSON compatibility, decoding, Results, tables, optionality, interfaces, aliases, and package changes remain out of scope. The next work is exactly ARRAY-M1 bounded runtime array encoding.
