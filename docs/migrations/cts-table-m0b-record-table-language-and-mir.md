...
The shared MIR validator checks primitive payload kind, record/field identities and completeness, duplicate fields, enum/case/payload shape, Result payload type and the void-success exclusion, column element agreement, rectangular row counts, and row-field identity/type agreement. Invalid table MIR returns shared `COPE-CS-0002`/`COPE-JS-0002` diagnostics with no artifact. CTS-TABLE-M1 realizes valid canonical table MIR in C# and CTS-TABLE-M2 realizes it in JavaScript.

The original bounded diagnostic inventory was: `0001` declaration/placement, `0002` collisions/compiler-owned bounds name, `0003` zero columns, `0004` duplicate column, `0005` untyped empty column, `0006` inferred heterogeneity, `0007` explicit mismatch, `0008` raggedness, `0009` non-constant or mutable cell, `0010` recursive authored data, `0011` invalid table use, `0012` invalid column, `0013` non-number index, `0014` table mutation, `0015` column mutation, `0016` row construction/update/mutation, `0017` table/row/column equality, `0018` nominal-row mismatch, and `0019` unresolved row/column annotation. CTS-TSON-WORKBOOK-M0 later narrowed `0014` to assignment/member mutation and admitted whole-column immutable `with`; `0020` rejects an empty table update, `0021` a duplicate replacement, and `0022` a non-authored replacement array. Rectangularity, cell closure, and element types continue to use `0008`, `0009`, and ordinary type diagnostics. `TableDiagnosticInventoryTests` and `TableFeatureTests` give the source cases non-empty spans. Source diagnostics remain distinct from malformed-MIR diagnostics.
...
The CLI writes only after successful compilation. A failed backend compilation preserves an existing output; success is determined by the command result, not the existence of an older file. CTS-TABLE-M1 and CTS-TABLE-M2 add C# and JavaScript table success respectively. M0b itself does not implement JSON, queries, builders, count/iteration, or general array indexing.
