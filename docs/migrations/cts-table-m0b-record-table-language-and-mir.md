...
The CLI writes only after successful compilation. A failed backend compilation preserves an existing output; success is determined by the command result, not the existence of an older file. CTS-TABLE-M1 adds C# table success while JavaScript table emission continues its stale-artifact-preserving rejection. M0b itself does not implement JSON, queries, builders, count/iteration, or general array indexing.
