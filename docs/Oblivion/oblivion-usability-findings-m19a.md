...
After M19a, normal work needs no implementation source. M19b subsequently made card-local artifact identity explicit through a typed workspace/page/card/artifact address and exposed resolution through the product surface.
...
Card provenance is strong enough for code-first edits: the card TOML and referenced Markdown are distinct and explicit. M19b now composes artifact declaration provenance with owner/producer provenance and exposes safe resolved location, existence, payload metadata, and scoped identity. Repeated `workspace-manifest` and `presenter-proof` local IDs remain valid because their addresses differ by owner.
...
## M19b completion note

M19b completed the following recorded scope:
...
The exact next scope is recorded in `oblivion-artifact-usability-m19b.md`.
