...
- exploratory `src/Aurelian.Shaders/Language/VdMir` code and `artifacts/m14a` remain in-tree from earlier compiler-only work
...
- `Copeland.slnx` and `Copeland.Slow.slnx` still exclude Aurelian projects
...
- `Copeland.slnx` and `Copeland.Slow.slnx` test/build confirmation
...
- add Aurelian projects to `Copeland.slnx` or `Copeland.Slow.slnx`
