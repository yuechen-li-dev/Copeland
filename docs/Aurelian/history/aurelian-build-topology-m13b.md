...
- `Aurelian.slnx` referenced a missing sample project, `samples/Aurelian/Aurelian.VisibleTriangle/Aurelian.VisibleTriangle.csproj`.
- `Aurelian.slnx` also referenced missing `vendor/Dominatus/src/...` projects.
- `src/Aurelian/Aurelian.Runtime/Aurelian.Runtime.csproj` project-referenced `../../vendor/Dominatus/src/Dominatus.Core/Dominatus.Core.csproj`.
...
- removes the missing `samples/Aurelian/Aurelian.VisibleTriangle/Aurelian.VisibleTriangle.csproj` entry from `Aurelian.slnx`
...
The visible-triangle sample remained deferred rather than recreated as a placeholder in this milestone. It now exists again under `samples/Aurelian/Aurelian.VisibleTriangle`, but that restoration happens after M13b and does not change the M13b topology doctrine. M13g later re-audits the restored sample and re-adds it to `Aurelian.slnx` because the project is present and builds cleanly.
...
- `dotnet test Machina.UI.Slow.slnx`: passed
...
- `rg -n "Aurelian.Graphics|Aurelian.Runtime|Aurelian.Vulkan|Vulkan" src/Machina.UI/Machina.Core src/Machina.UI/Machina.Standard src/Machina.UI/Machina.Layout src/Machina.UI/Machina.Runtime src/Machina.UI/Machina.Pipeline`
- `rg -n "Aurelian" Copeland.slnx Machina.UI.Slow.slnx`
