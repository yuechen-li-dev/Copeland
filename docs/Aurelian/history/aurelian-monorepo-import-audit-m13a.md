...
The imported compiler surface is concentrated in `src/Aurelian/Aurelian.Shaders`:
...
- `Machina.UI.Slow.slnx` remains the current slow/proof solution.
...
- `src/Aurelian/Aurelian.Runtime/Aurelian.Runtime.csproj` referenced `../../vendor/Dominatus/src/Dominatus.Core/Dominatus.Core.csproj`, but the repo uses `reference/dominatus` for inspection only and no active `vendor/` tree.
...
- `Aurelian.slnx` stays separate from `Copeland.slnx` and `Machina.UI.Slow.slnx`.
