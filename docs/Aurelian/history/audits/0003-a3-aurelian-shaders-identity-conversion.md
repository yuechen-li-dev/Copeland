...
- Renamed `src/StriV.ShaderPipeline/` to `src/Aurelian/Aurelian.Shaders/`.
- Renamed `src/StriV.ShaderPipeline/StriV.ShaderPipeline.csproj` to `src/Aurelian/Aurelian.Shaders/Aurelian.Shaders.csproj`.
- Updated production namespaces and imports under `src/Aurelian/Aurelian.Shaders/` from `StriV.ShaderPipeline` to `Aurelian.Shaders`.
- Added `tests/Aurelian/Aurelian.Shaders.Tests/Aurelian.Shaders.Tests.csproj`.
- Added `tests/Aurelian/Aurelian.Shaders.Tests/ShaderPipelineIdentityTests.cs`.
...
```text
src/Aurelian/Aurelian.Shaders/Aurelian.Shaders.csproj
```
...
All production C# namespaces and intra-project imports under `src/Aurelian/Aurelian.Shaders/` were renamed from `StriV.ShaderPipeline` to `Aurelian.Shaders`.
...
```text
src/Aurelian/Aurelian.Shaders/Aurelian.Shaders.csproj
tests/Aurelian/Aurelian.Shaders.Tests/Aurelian.Shaders.Tests.csproj
```
...
```text
tests/Aurelian/Aurelian.Shaders.Tests/
```
...
Boundary checks were run against `src/Aurelian/Aurelian.Shaders` and `tests/Aurelian/Aurelian.Shaders.Tests` for old identity strings and forbidden reference names:

```bash
rg -n "StriV\.ShaderPipeline|namespace StriV|StriV.Core.Profile|CodeReferences|Stride\.|Machina\.|WyrmCoil|Copeland" src/Aurelian/Aurelian.Shaders tests/Aurelian/Aurelian.Shaders.Tests -g '*.cs' -g '*.csproj' || true
```
...
The inventory shows `tests/Aurelian/Aurelian.Shaders.Tests` references `src/Aurelian/Aurelian.Shaders`. It also shows pre-existing references in the unlinked salvage projects under `src/StriV.AssetPipeline` and `src/StriV.AssetTool`; those projects were intentionally not modified in A3.
