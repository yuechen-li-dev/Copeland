...
* Updated `samples/Aurelian/Aurelian.VisibleTriangle/Program.cs` to replace the manual top-level one-frame pump call with a finite `AurelianFrameLoop` that uses a Dominatus-backed runtime tick frame step and prepared presentation mechanism.
* Added `samples/Aurelian/Aurelian.VisibleTriangle/VisibleTriangleFrameInputProvider.cs` as the sample-local `IAurelianFrameInputProvider` for the prepared frame input.
* Updated `samples/Aurelian/Aurelian.VisibleTriangle/README.md` to describe the A66 frame-loop/runtime-session sample path and one-frame M0 limitation.
...
dotnet build samples/Aurelian/Aurelian.VisibleTriangle/Aurelian.VisibleTriangle.csproj -c Debug
```
...
```bash
rg -n "Aurelian.Shaders|Dxc|DXC|Microsoft.Direct3D.DXC|SDSL|Sdslv|Hlsl|SpirvShaderArtifact" samples/Aurelian/Aurelian.VisibleTriangle src/Aurelian/Aurelian.Graphics -g '*.cs' -g '*.csproj' || true
rg -n "Aurelian.Host|ServiceLocator|Singleton|Activator|GetType\\(|Type\\.|Vortice|VMASharp|Vma|CodeReferences|Stride\\.|Machina\\.|WyrmCoil|Copeland" samples/Aurelian/Aurelian.VisibleTriangle src tests -g '*.cs' -g '*.csproj' || true
rg -n "AurelianFrameLoop|AurelianRuntimeSession|AurelianRuntimeTickFrameStep|AurelianRuntimeSessionTickerAdapter|RunOneFrameAsync" samples/Aurelian/Aurelian.VisibleTriangle -g '*.cs' || true
rg -n "ProjectReference" samples/Aurelian/Aurelian.VisibleTriangle src/Aurelian/Aurelian.Core src/Aurelian/Aurelian.Runtime src/Aurelian/Aurelian.Graphics src/Aurelian/Aurelian.Rendering.Contracts -g '*.csproj'
```
