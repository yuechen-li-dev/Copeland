...
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPlantOutputImage.cs`: non-owning plant output wrapper over offscreen `AurelianVulkanTexture` sources.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPlantOutputImageSet.cs`: output collection and lookup helpers.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPlantOutputResolver.cs`: neutral `PlantOutputRef` to backend source resolver.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPlantOutputResolutionResult.cs`: typed plant-output resolution result.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPlantOutputStatus.cs`: plant-output resolver status enum.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPlantOutputDiagnostic.cs`: typed plant-output diagnostics.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPlantOutputDiagnosticCodes.cs`: A53 plant-output diagnostic code constants.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanCompositorPassthrough.cs`: mechanism-only Vulkan passthrough compositor dispatch path.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanCompositorResult.cs`: typed mechanism result plus neutral dispatch result.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanCompositorStatus.cs`: mechanism status enum.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanCompositorDiagnostic.cs`: typed mechanism diagnostics.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanCompositorDiagnosticCodes.cs`: A53 compositor diagnostic code constants.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Resources/Barriers/VulkanPresentationTargetBarrierEmission.cs`: presentation-target image barrier emission record.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Resources/Barriers/VulkanBarrierCommandEmitter.cs`: minimal presentation-target image barrier emission support.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Commanding/Submit/VulkanCommandSubmitter.cs`: exposes the last known command-list fence value for command-buffer rental.
- `tests/Aurelian/Aurelian.Graphics.Tests/VulkanPlantOutputImageM0Tests.cs`: headless-safe plant-output wrapper/resolver tests.
- `tests/Aurelian/Aurelian.Graphics.Tests/VulkanCompositorPassthroughM0Tests.cs`: headless-safe compositor rejection and optional real-copy tests.
- `tests/Aurelian/Aurelian.Graphics.Tests/Aurelian.Graphics.Tests.csproj`: enables unsafe helper construction for synthetic texture wrapper tests.
...
- `dotnet test tests/Aurelian/Aurelian.Graphics.Tests/Aurelian.Graphics.Tests.csproj -c Debug --no-restore`
