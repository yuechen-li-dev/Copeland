...
- Added `src/Aurelian/Aurelian.Graphics/Vulkan/Resources/Buffers/` with Aurelian-owned buffer usage flags, create plans, diagnostics, create results, native buffer ownership, and the buffer factory.
- Added `tests/Aurelian/Aurelian.Graphics.Tests/VulkanBufferM0Tests.cs` for validation, unavailable-safe creation, disposal, and allocator-boundary checks.
...
- `dotnet test tests/Aurelian/Aurelian.Graphics.Tests/Aurelian.Graphics.Tests.csproj -c Debug` passes.
