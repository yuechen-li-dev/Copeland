...
- `src/Aurelian/Aurelian.Graphics/Aurelian.Graphics.csproj` creates the first graphics HAL project boundary.
- `src/Aurelian/Aurelian.Graphics/AurelianGraphicsProject.cs` adds the project identity smoke type.
- `src/Aurelian/Aurelian.Graphics/Vulkan/VulkanPackageSmoke.cs` adds package/API smoke access for Silk.NET Vulkan and windowing types.
- `src/Aurelian/Aurelian.Graphics/Plants/` and `src/Aurelian/Aurelian.Graphics/Vulkan/*/` add the planned graphics HAL folder scaffold.
- `tests/Aurelian/Aurelian.Graphics.Tests/Aurelian.Graphics.Tests.csproj` creates the graphics test project.
- `tests/Aurelian/Aurelian.Graphics.Tests/AurelianGraphicsProjectTests.cs` covers project identity, package smoke values, and direct forbidden assembly references.
...
```text
src/Aurelian/Aurelian.Graphics/
```

Created test scaffold:

```text
tests/Aurelian/Aurelian.Graphics.Tests/
```

Created planned folder structure:

```text
src/Aurelian/Aurelian.Graphics/Plants/
src/Aurelian/Aurelian.Graphics/Vulkan/Device/
src/Aurelian/Aurelian.Graphics/Vulkan/Sync/
src/Aurelian/Aurelian.Graphics/Vulkan/Resources/
src/Aurelian/Aurelian.Graphics/Vulkan/Commanding/
src/Aurelian/Aurelian.Graphics/Vulkan/Pipelines/
src/Aurelian/Aurelian.Graphics/Vulkan/Presentation/
src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/
src/Aurelian/Aurelian.Graphics/Vulkan/Diagnostics/
```
...
Added `tests/Aurelian/Aurelian.Graphics.Tests/AurelianGraphicsProjectTests.cs` with:
...
```bash
rg -n "Vortice|VMASharp|Vma|vkCreateInstance|CreateVulkanSurface|IWindow|Window.Create|new Vk|Vk.GetApi|CreateDevice|vkCreateDevice|SwapChain|Swapchain|GraphicsDevice" src/Aurelian/Aurelian.Graphics tests/Aurelian/Aurelian.Graphics.Tests -g '*.cs' -g '*.csproj' || true
```

Result: no matches.

```bash
rg -n "Aurelian.World|Aurelian.Assets|Aurelian.Shaders|Aurelian.Rendering.Null|Dominatus|CodeReferences|Stride\.|Machina\.|WyrmCoil|Copeland" src/Aurelian/Aurelian.Graphics tests/Aurelian/Aurelian.Graphics.Tests -g '*.cs' -g '*.csproj' || true
```
...
Result: only `Silk.NET.Vulkan` and `Silk.NET.Windowing` appear in `Directory.Packages.props` and `src/Aurelian/Aurelian.Graphics/Aurelian.Graphics.csproj`. No `Silk.NET.Core`, Vortice, or VMASharp references appear.

```bash
rg -n "ProjectReference" src/Aurelian/Aurelian.Graphics tests/Aurelian/Aurelian.Graphics.Tests -g '*.csproj'
```
