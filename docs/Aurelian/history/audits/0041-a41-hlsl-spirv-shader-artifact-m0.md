...
- Added `Aurelian.Shaders.Language.Artifacts.Spirv` model, diagnostics, emitter, and JSON writer files under `src/Aurelian/Aurelian.Shaders/Language/Artifacts/Spirv/`.
- Added `tests/Aurelian/Aurelian.Shaders.Tests/SpirvShaderArtifactM0Tests.cs` for validation, DXC availability-safe compilation, hashes, JSON behavior, and graphics-runtime boundary checks.
...
- `tests/Aurelian/Aurelian.Shaders.Tests/Fixtures/Hlsl/tiny_triangle_vs.hlsl` as `Vertex`, `VSMain`, `vs_6_0`.
- `tests/Aurelian/Aurelian.Shaders.Tests/Fixtures/Hlsl/tiny_triangle_ps.hlsl` as `Fragment`, `PSMain`, `ps_6_0`.
...
dotnet test tests/Aurelian/Aurelian.Shaders.Tests/Aurelian.Shaders.Tests.csproj -c Debug
...
rg -n "SpirvShaderArtifact|spirvBase64|HlslShaderStageSource|DxcSpirvCompiler" src/Aurelian/Aurelian.Graphics src/Aurelian/Aurelian.Runtime src/Aurelian/Aurelian.World src/Aurelian/Aurelian.Rendering.Contracts src/Aurelian/Aurelian.Rendering.Null -g '*.cs' -g '*.csproj' || true
```
