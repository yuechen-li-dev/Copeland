...
- Added Vulkan init diagnostics and facts under `src/Aurelian/Aurelian.Graphics/Vulkan/Diagnostics/`.
- Added Vulkan plant options, initializer, and native handle owner under `src/Aurelian/Aurelian.Graphics/Vulkan/Device/`.
- Enabled unsafe blocks in `src/Aurelian/Aurelian.Graphics/Aurelian.Graphics.csproj` for the Vulkan interop boundary only.
- Added unavailable-safe Vulkan initialization tests in `tests/Aurelian/Aurelian.Graphics.Tests/VulkanPlantInitializerM0Tests.cs`.
...
rg -n "queueFamilyIndex\s*=\s*0|QueueFamilyIndex\s*=\s*0" src/Aurelian/Aurelian.Graphics -g '*.cs' || true
