...
- Extended `src/Aurelian/Aurelian.Assets/AssetPipeline.cs` with asset-manifest shader artifact references, diagnostics, parsing, and validation.
- Added `src/Aurelian/Aurelian.Assets/Shaders/LoadedShaderAsset.cs` for loaded shader asset records.
- Added `src/Aurelian/Aurelian.Assets/Shaders/ShaderAssetManifestLoadResult.cs` for aggregate manifest shader load results.
- Added `src/Aurelian/Aurelian.Assets/Shaders/ShaderAssetManifestLoader.cs` to parse asset manifests, validate shader references, resolve artifact paths, and load shader artifacts.
- Added `tests/Aurelian/Aurelian.Assets.Tests/AssetManifestShaderReferencesM0Tests.cs` for parser, validation, loader, failure mapping, and graphics-boundary coverage.
- Added `samples/Aurelian/Aurelian.VisibleTriangle/Assets/assets.toml` and updated the visible sample project so the manifest is copied to output.
- Updated `README.md`, `docs/architecture/mvp-roadmap.md`, `docs/architecture/dependency-policy.md`, and `samples/Aurelian/Aurelian.VisibleTriangle/README.md` for A70 behavior and boundaries.
...
```text
samples/Aurelian/Aurelian.VisibleTriangle/Assets/assets.toml
```
...
dotnet test tests/Aurelian/Aurelian.Assets.Tests/Aurelian.Assets.Tests.csproj -c Debug
...
- update `samples/Aurelian/Aurelian.VisibleTriangle` to load `assets.toml`;
