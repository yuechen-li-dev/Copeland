...
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPresentationTargetImage.cs`: non-owning swapchain presentation image wrapper.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPresentationTargetImageSet.cs`: ordered wrapper set with image-index lookup and swapchain construction helper.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPresentationTargetResolver.cs`: neutral `PresentationTargetRef` to backend wrapper resolver.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPresentationTargetResolutionResult.cs`: typed resolution result.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPresentationTargetStatus.cs`: typed resolver status enum.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPresentationTargetDiagnostic.cs`: typed resolver diagnostics.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor/VulkanPresentationTargetDiagnosticCodes.cs`: A52 diagnostic codes.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Presentation/AurelianVulkanSwapchain.cs`: public image-set creation method and disposed-state check.
- `src/Aurelian/Aurelian.Graphics/Properties/AssemblyInfo.cs`: internal visibility for focused graphics tests.
- `tests/Aurelian/Aurelian.Graphics.Tests/VulkanPresentationTargetImageM0Tests.cs`: headless-safe wrapper and resolver tests.
...
`tests/Aurelian/Aurelian.Graphics.Tests/VulkanPresentationTargetImageM0Tests.cs` adds headless-safe coverage for:
...
rg -n "Aurelian.World|Aurelian.Rendering.Null|Aurelian.Shaders|Dxc|DXC|SDSL|Sdslv|Hlsl|Vortice|VMASharp|Vma|CodeReferences|Stride\.|Machina\.|WyrmCoil|Copeland|ServiceLocator|Singleton|Activator|GetType\(|Type\." src/Aurelian/Aurelian.Graphics tests/Aurelian/Aurelian.Graphics.Tests -g '*.cs' -g '*.csproj' || true
rg -n "vkCmdCopyImage|CmdCopyImage|vkCmdBlitImage|CmdBlitImage|vkCmdPipelineBarrier|CmdPipelineBarrier|vkQueueSubmit|QueueSubmit|vkQueuePresent|QueuePresent|vkAcquireNextImage|AcquireNextImage" src/Aurelian/Aurelian.Graphics/Vulkan/Compositor tests/Aurelian/Aurelian.Graphics.Tests -g '*.cs' || true
rg -n "DestroyImage|vkDestroyImage|DestroyImageView|vkDestroyImageView" src/Aurelian/Aurelian.Graphics/Vulkan/Compositor tests/Aurelian/Aurelian.Graphics.Tests -g '*.cs' || true
```
...
The destroy-boundary search reports no destroy image/image-view calls under `src/Aurelian/Aurelian.Graphics/Vulkan/Compositor`; swapchain image-view destruction remains in the existing swapchain owner.
...
- `dotnet test tests/Aurelian/Aurelian.Graphics.Tests/Aurelian.Graphics.Tests.csproj -c Debug`: passed.
