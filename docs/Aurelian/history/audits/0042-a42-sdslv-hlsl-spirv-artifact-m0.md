...
- Added `src/Aurelian/Aurelian.Shaders/Language/Artifacts/SdslvSpirv/` with the SDSL-V SPIR-V artifact model, diagnostics, M0 stage extraction, emitter, status enum, and deterministic JSON writer.
- Updated `src/Aurelian/Aurelian.Shaders/Language/Emission/Hlsl/HlslEmitter.cs` with the minimal DXC-friendly smoke-path semantic conventions required by A42.
- Added `tests/Aurelian/Aurelian.Shaders.Tests/SdslvSpirvArtifactM0Tests.cs` and updated the HLSL emission smoke expectation for semantic-bearing fields.
...
rg -n "SdslvSpirvShaderArtifact|SpirvShaderArtifact|HlslShaderStageSource|DxcSpirvCompiler" src/Aurelian/Aurelian.Graphics src/Aurelian/Aurelian.Runtime src/Aurelian/Aurelian.World src/Aurelian/Aurelian.Rendering.Contracts src/Aurelian/Aurelian.Rendering.Null -g '*.cs' -g '*.csproj' || true
git status --short
```
