...
- Added `src/Aurelian/Aurelian.Graphics/Vulkan/Resources/Textures/` for texture usage flags, texture format vocabulary, create plans, diagnostics, create results, native texture ownership, and texture factory creation.
- Added `tests/Aurelian/Aurelian.Graphics.Tests/VulkanTextureM0Tests.cs` for validation, unavailable-safe Vulkan creation, layout tracker initialization, disposal idempotence, and allocator-boundary checks.
