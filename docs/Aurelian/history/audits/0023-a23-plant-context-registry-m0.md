...
- Added native-free plant model files under `src/Aurelian/Aurelian.Graphics/Plants/`:
  - `PlantId.cs`
  - `PlantKind.cs`
...
- Added deterministic plant registry tests in `tests/Aurelian/Aurelian.Graphics.Tests/PlantRegistryM0Tests.cs`.
...
rg -n "Silk|Vulkan|Vk|IWindow|WindowOptions|nint|IntPtr|unsafe|Vk[A-Z]|Handle|Device|Queue|Surface|Swapchain|CommandBuffer|vkCreate|Vk.GetApi|GetApi" src/Aurelian/Aurelian.Graphics/Plants tests/Aurelian/Aurelian.Graphics.Tests/PlantRegistryM0Tests.cs -g '*.cs' || true
