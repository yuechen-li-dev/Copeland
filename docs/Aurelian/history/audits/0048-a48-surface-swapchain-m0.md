...
- `Directory.Packages.props` and `src/Aurelian/Aurelian.Graphics/Aurelian.Graphics.csproj`: added the Silk.NET KHR Vulkan extension package used for `VK_KHR_surface` and `VK_KHR_swapchain` commands.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Device/*`: added presentation-aware plant initialization through `VulkanPlantOptions.EnablePresentation`.
- `src/Aurelian/Aurelian.Graphics/Vulkan/Presentation/*`: added presentation diagnostics, facts, owners, factory, selection helpers, and acquire/present deferred skeletons.
- `tests/Aurelian/Aurelian.Graphics.Tests/VulkanSwapchainM0Tests.cs`: added headless-safe swapchain M0 tests.
