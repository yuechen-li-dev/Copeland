...
- Added `samples/Aurelian/Aurelian.VisibleTriangle/VisibleTriangleShaderAssets.cs` as the sample-local manifest shader loading helper.
- Updated `samples/Aurelian/Aurelian.VisibleTriangle/VisibleTriangleSampleFrame.cs` so prepared Vulkan setup receives an already-loaded `CompiledShaderProgram` instead of directly loading a shader artifact TOML path.
- Updated `samples/Aurelian/Aurelian.VisibleTriangle/Program.cs` so startup loads the shader through `Assets/assets.toml` before creating the Vulkan sample frame.
- Updated `samples/Aurelian/Aurelian.VisibleTriangle/README.md`, `README.md`, `docs/architecture/mvp-roadmap.md`, and `docs/architecture/dependency-policy.md` for A71 behavior and boundaries.
