...
- Added `src/Aurelian/Aurelian.Graphics/Vulkan/Pipelines/Framebuffers/` with Aurelian-owned framebuffer descriptor, diagnostics, result/status, factory, and native owner.
- Added `tests/Aurelian/Aurelian.Graphics.Tests/VulkanFramebufferM0Tests.cs`.
...
rg -n "vkCreateFramebuffer|CreateFramebuffer|vkDestroyFramebuffer|DestroyFramebuffer" src/Aurelian/Aurelian.Graphics/Vulkan/Pipelines/Framebuffers tests/Aurelian/Aurelian.Graphics.Tests -g '*.cs' || true
git status --short
```
