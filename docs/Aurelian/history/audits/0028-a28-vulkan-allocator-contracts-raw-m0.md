...
- Added `src/Aurelian/Aurelian.Graphics/Vulkan/Resources/Allocation/` with allocator contracts, allocation handle, telemetry, diagnostics, resource state, and the raw Vulkan allocator backend.
- Added `tests/Aurelian/Aurelian.Graphics.Tests/VulkanMemoryAllocatorM0Tests.cs` for unavailable-safe allocator validation.
