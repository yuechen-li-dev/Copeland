...
## M20 evidence update — 2026-09-02

TINY-FARM-M20 turns the M16 hotbar into a capability selector with one bounded identity tool. Axe is an existing `ItemId`/`ItemState` owned by the player; Wood is an existing stackable `ProductId`. The hotbar is now the closed sum Product or Item, with the prior Turnip Seed and Turnip product bindings unchanged and Axe in slot 3. Selection is persisted semantic player state, but it never grants capability: item availability is derived from authoritative ownership.

One TSON-authored Farm Tree has a concrete `TreeDefinition` and a separately persisted Standing/Depleted `TreeState`; it is intentionally not flattened into the bare-hand M18 ForageNode. The shared target order is actor, portal, ground item, forage, tree, plot, cooking station, shop, then squared distance and stable ID. Q/UseSelected with selected Axe and a standing Tree lowers through one closed branch to `ChopIntent(TreeId)`. E/Interact never chops. Wrong selection returns `WrongTool`, and missing ownership returns `SelectedBindingUnavailable` before lowering.

`ChopIntent` remains independently authoritative for direct/replay controllers: the resolver revalidates player actor, Axe ownership, Tree identity/scene/range/standing state, and fixed authored output before atomically adding Wood and depleting the Tree. Axe is neither consumed nor modified. Version-9 persistence and semantic hashing retain Tree state; existing item, stack, and selected-slot schemas retain Axe/Wood/hotbar truth. Frame/UI/DTO project a stump, `Wood x1`, selected Axe, and Tree status without renderer mutation.

The reusable architectural seam is now proven rather than speculative: `selected semantic capability + semantic target -> typed intent -> resolver-owned mutation`. A future `selected weapon + enemy -> AttackIntent` should deliberately mirror Axe/Tree/Chop, but M20 adds no Attack, combat, tool interface, capability registry, generic resource node, durability, stats, skills, stamina, timing, hits, RNG, respawn, or growth. Detailed evidence is in `docs/Aurelian/games/tiny-farm-m20-axe-woodcutting-report.md` and `artifacts/tiny-farm-m20/`.

## M21 evidence update — 2026-09-02

TINY-FARM-M21 proves that the existing scene/capability/intent/resolver architecture crosses into a hostile domain without contaminating peaceful scenes. `Old Burrow` is a normal 16×12 TSON-authored scene joined to Overworld through ordinary portal, anchor, and route semantics. One TSON-authored `Slime` has a concrete `EnemyDefinition`; its save state contains only current health, with Alive/Defeated derived. Enemy is structurally separate from scheduled/social actors and does not enter Dominatus.

Sword is one identity-bearing `ItemState`, owned by the player and bound to slot 4. Living Enemy participates in the shared facing/range target law with exact priority `actor, enemy, portal, ground item, forage, tree, plot, cooking station, shop`. Friendly actors remain non-enemies. Sword + Enemy lowers through the closed `UseSelectedIntent` map to `AttackIntent(EnemyId)`; direct Attack independently revalidates player authority, Sword ownership, enemy identity, scene, lifecycle, facing, and range. One valid reduction changes HP 1→0 and emits one `EnemyDefeated`; stale Attack returns `AlreadyDefeated` without mutation.

There is no combat mode, player health, enemy movement/attack, cooldown, loot, XP, faction, ECS, generic combat/action/health framework, or renderer mutation. The Slime is deliberately nonblocking until dynamic-body semantics have real pressure. Version-10 save/hash, snapshot v6, CLI/LLM commands, replay, and frame/UI projections preserve the same ownership boundaries. Peaceful Farm qualification emits no combat event or enemy policy work.

The M1–M21 checkpoint keeps gameplay semantics local, keeps closed lowering, keeps TSON and Dominatus boundaries, defers generic producer/combat/entity abstractions, and recommends only two bounded cleanups: consolidate repeated loader/scenario plumbing, and build the missing Machina.UI presentation/input adapter without moving gameplay truth. Detailed evidence and the twenty-question checkpoint are in `docs/Aurelian/games/tiny-farm-m21-first-dungeon-combat-report.md` and `artifacts/tiny-farm-m21/`.
