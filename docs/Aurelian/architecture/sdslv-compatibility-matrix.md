...
| Test modules/runner | Reference includes rich tests and prototype `.sdslvtest` support. | xUnit parser/AST/validation/emission/artifact tests in Aurelian, including the checked-in `tests/Aurelian/Aurelian.Shaders.Tests/Fixtures/Sdslv/smoke_triangle.sdslv` source fixture. | Intentionally different | A9 still intentionally uses ordinary xUnit tests only; `.sdslvtest` and CPU evaluator work remain deferred. |
