...
- add a minimal `VD-MIR M0` model under `src/Aurelian.Shaders/Language/VdMir`
...
The runtime boundary is preserved. M14a writes proof artifacts only under `artifacts/m14a` and does not rewrite `samples/Aurelian.VisibleTriangle/Assets/Shaders/SmokeTriangle/*`. The sample still loads its existing checked-in `shader.toml`, `generated.hlsl`, `VSMain.spv.hex`, and `PSMain.spv.hex`.
...
- added `src/Aurelian.Shaders/Language/VdMir/*`
