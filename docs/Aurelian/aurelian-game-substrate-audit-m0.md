...
|---|---|---|---|
| 1 | Simulation host/clocks | B; A kernel ticking | `TinyFarmSimulationHost` separates accepted host ticks, observed render frames, 60 Hz locomotion, world minutes and NPC decisions; bounded catch-up and pause/play/fast-forward. `AurelianRuntimeSession` and `AiClock` already tick decisions. Extract only ordered multi-rate cadence when wiring the next real consumer; do not move game-minute commands/session callbacks wholesale. |
| 2 | Scene/world runtime | B; A world stores | TinyFarm scene IDs, validated tables, anchors, routes and resolver transitions work. Aurelian `WorldDocument`, typed stores and actuation are reusable but not an equivalent room runtime. Active-scene fixed movement and inactive coarse NPC handoff are game policy. Formalize catalog/anchor/query adapters; leave entities local. |
...
| 6 | Collision/physics | A local tile blocking; C reusable queries | TinyFarm cardinal point movement tests blocked tiles through the authoritative resolver. This is not an actor-volume sweep/slide substrate. Need AABB/circle overlap, static-map sweep/slide and trigger edge facts first. Raycast for demonstrated projectile/line-of-sight pressure; capsule/simple polygon later. No need for Box2D-class dynamics now. |
...
| 10 | Audio | C native runtime; A generation services | `Dominatus.Actuators.Audio` generates artifacts with providers/metadata; Godot playback depends on Godot. No Aurelian mixer/voice lifetime/music streaming/buses found. High priority: SFX, music/ambient loops, categories, volume, fade/crossfade, simple pan/attenuation and event deduplication. TTS generation is not playback infrastructure. |
...
| 19 | Navigation | A algorithm; B scene adapter | `DotRecastNavigationPlanner` builds/caches scene walkable geometry and emits canonical paths; semantic anchor intents hand off through session/resolver. Adequate for the slice after adapting scene data. Do not add another grid pathfinder. Nav paths are proposals; collision/resolver still accepts movement. |
| 20 | NPC scheduling | A decision kernel; B author bridge | Required/Open schedule windows, TSON candidate tables, utility and persistent per-NPC action flows work in TinyFarm. “Work at 9, wander at noon, sleep at night” is already table authoring there; a new game still reconstructs observations, window lookup, goal-to-anchor handoff, arrival/failure and active/inactive cadence. Extract that bridge only, not energy or jobs. |
