...
- Aurelian source projects are now present under `src/Aurelian/Aurelian.*`.
...
- `samples/Aurelian/Aurelian.VisibleTriangle` exists as a future proof target but is not wired to `VD-MIR` in M13f
