...
Canonical app authoring now defaults to `StandardUI` component helpers for controls/components. `StandardView` remains for lightweight row metadata, and sub-part helpers (`CheckboxBox`, `SwitchTrack`, `SwitchThumb`) are advanced/manual composition helpers only. See `docs/Machina.UI/history/standard-ui-vs-standard-view-m5d.md`.
...
M7a adds `samples/Machina.UI/Machina.ComponentGallery.Sample` as the canonical StandardUI visual workbench.
