...
Hybrid authoring remains preferred for app screens: flat top-level placement + hosted `StandardUI` components. Manual row decomposition of checkbox/switch internals is now considered advanced customization only and is no longer default guidance. See `docs/Machina.UI/history/standard-ui-vs-standard-view-m5d.md`.
