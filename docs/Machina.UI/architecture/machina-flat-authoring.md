...
Flat row authoring remains valid. For normal control usage, prefer hosted `StandardUI` controls at app level, and reserve `StandardView.CheckboxBox`/`SwitchTrack`/`SwitchThumb` for advanced custom component work. See `docs/Machina.UI/history/standard-ui-vs-standard-view-m5d.md`.
