...
`samples/Machina.UI/Machina.Presenter.Sample` now delegates frame construction to `MachinaRasterPipeline` and keeps only presenter concerns (windowing, bitmap conversion, pointer input, and state).
