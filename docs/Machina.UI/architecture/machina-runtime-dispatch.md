...
As of M5b, `samples/Machina.UI/Machina.Presenter.Sample` uses plain C# dispatch (`if` branches over typed `UiActionId`) for local hand-authored `DemoState` transitions.
