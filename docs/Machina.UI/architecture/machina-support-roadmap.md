...
| Avalonia token-anchored text conformance | Machina.Fonts + isolated Avalonia oracle | Implemented, Outcome B | Normal fonts tests + dedicated 16/24/32/48/64 runner + compact JSON | Avalonia is external layout/raster truth; DirectOutline and MSDF share `MachinaGlyphRun`. Layout/anchors converge, while MSDF remains experimental pending bounded field/reconstruction defects. |
| Avalonia/Skia positioned-outline conformance | Machina.Fonts + isolated Avalonia/Skia oracle | Implemented, Outcome A | Vector-only 64/96/128 fox runner + held-out strings + compact JSON + local SVG overlays | Natural cumulative placement, glyph IDs, baseline, scale, advances, transformed bounds, and vector outlines coincide. Raster/MSDF work was frozen during M1; MSDF realization may resume against this qualified upstream law. |
