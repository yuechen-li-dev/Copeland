...
New reference-rendering code lives under `src/Machina.UI/Machina.Fonts/ReferenceRendering/`.
...
M8l now builds on this proof by exporting and visually auditing a deterministic text proof set, documenting the current Y-orientation choice, and leaving broader renderer integration deferred. See `docs/Machina.UI/history/machina-cpu-msdf-text-proof-audit-m8l.md`.
