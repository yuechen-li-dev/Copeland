...
M12a adds a small deterministic Markdown frontend under `src/Copeland.Markdown`.
...
dotnet run --project src/Copeland.Cli -- markdown export-corpus --output-dir artifacts\m12a
```
...
```powershell
dotnet run --project src/Copeland.Cli -- markdown parse docs/machina-oblivion-phase-closeout-m11g.md --emit ast
dotnet run --project src/Copeland.Cli -- markdown parse docs/machina-oblivion-phase-closeout-m11g.md --emit mir --format json
```
