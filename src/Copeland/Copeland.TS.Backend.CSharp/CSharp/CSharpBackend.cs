...
public static class CSharpBackend
{
    ...
    private static void EmitFunction(CSharpTextWriter writer, MirFunction function, List<CSharpDiagnostic> diagnostics)
    {
    ...
    private static void EmitStatement(CSharpTextWriter writer, MirStatement stmt, MirFunction fn, ref int tempIndex, List<CSharpDiagnostic> diagnostics)
    {
        switch (stmt)
        {
            ...
            case MirIfStatement i:
                ...
                break;
            default:
                diagnostics.Add(new CSharpDiagnostic("COPE-CS-0001", $"Unsupported MIR statement: {stmt.GetType().Name}"));
                break;
        }
    }

    private static string EmitExpression(CSharpTextWriter writer, MirExpression expr, MirFunction fn, ref int tempIndex, List<CSharpDiagnostic> diagnostics)
    {
        return expr switch
        {
    ...
    private static string EmitEnumValueExpression(MirEnumValueExpression e, CSharpTextWriter writer, MirFunction fn, ref int tempIndex, List<CSharpDiagnostic> diagnostics)
    ...
    private static string EmitMatchExpression(MirMatchExpression m, CSharpTextWriter writer, MirFunction fn, ref int tempIndex, List<CSharpDiagnostic> diagnostics)
    {
    ...
    private static string AddUnsupportedExpression(string feature, List<CSharpDiagnostic> diagnostics)
    {
        diagnostics.Add(new CSharpDiagnostic("COPE-CS-0001", $"Unsupported MIR expression: {feature}"));
        return "default!";
    }

    private static string EmitPropagation(CSharpTextWriter writer, MirCallExpression c, MirFunction fn, ref int tempIndex, List<CSharpDiagnostic> diagnostics)
    {
        if (!fn.IsFallible || fn.ErrorType is null)
        {
            diagnostics.Add(new CSharpDiagnostic("COPE-CS-0002", "Propagation used in non-fallible function."));
            return "default!";
        }
    ...
    private static List<string> EmitArguments(IReadOnlyList<MirExpression> arguments, CSharpTextWriter writer, MirFunction fn, ref int tempIndex, List<CSharpDiagnostic> diagnostics)
    {
