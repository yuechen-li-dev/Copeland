...
public static class CSharpBackend
{
        ...
        bool supportsTableWith)
    {
        ...
        foreach (var column in table.Columns)
        {
            ...
            {
                writer.WriteLine();
                writer.WriteLine($"public CopeResult<{MapType(column.ElementType)}, TableBoundsError> At(double index) => Get(index);");
            }
            writer.Unindent();
            writer.WriteLine("}");
            writer.WriteLine();
        ...
        foreach (var column in table.Columns)
        {
            ...
            if (table.IsExported)
            {
                writer.WriteLine($"public {MapType(column.ElementType)} {CSharpNameMangler.Mangle(column.Name)} => _table.{TableReadMethodName(column.Id)}(_index);");
                writer.WriteLine($"internal {MapType(column.ElementType)} {TableRowFieldName(column.Id)} => {CSharpNameMangler.Mangle(column.Name)};");
            }
            else
            {
        ...
        if (supportsTableWith)
        {
            ...
            {
                writer.WriteLine($"internal {tableType} {TableWithColumnMethodName(replacementColumn.Id)}({MapType(replacementColumn.ElementType)}[] values)");
                writer.WriteLine("{");
                writer.Indent();
                writer.WriteLine($"return new {tableType}(");
                writer.Indent();
                ...
                {
                    MirTableColumnDefinition column = table.Columns[index];
                    string argument = column.Id == replacementColumn.Id
                        ? "values"
                        : TableStorageName(column.Id);
                    string comma = index == table.Columns.Count - 1 ? string.Empty : ",";
                    writer.WriteLine($"{argument}{comma}");
                }
                writer.Unindent();
                writer.WriteLine(");");
                writer.Unindent();
        ...
        if (table.IsExported)
        {
            writer.WriteLine($"public int RowCount => {table.RowCount};");
        }
        foreach (var column in table.Columns)
        {
            writer.WriteLine();
            if (table.IsExported)
            {
                writer.WriteLine($"public {TableColumnTypeName(column.Id)} {CSharpNameMangler.Mangle(column.Name)} => {TableColumnPropertyName(column.Id)};");
            }
        ...
        {
            writer.WriteLine();
            writer.WriteLine($"public CopeResult<{rowType}, TableBoundsError> At(double index) => GetRow(index);");
        }
        writer.Unindent();
        writer.WriteLine("}");
        writer.WriteLine();
    ...
    private static void EmitBoundsCheckedElementAccess(
        CSharpTextWriter writer,
        MirType elementType,
        string storage,
        int rowCount,
        string accessibility = "internal override")
    {
        string elementTypeName = MapType(elementType);
        writer.WriteLine($"{accessibility} CopeResult<{elementTypeName}, TableBoundsError> Get(double index)");
        writer.WriteLine("{");
        writer.Indent();
        writer.WriteLine("if (double.IsNaN(index) || double.IsInfinity(index) || index != global::System.Math.Truncate(index))");
        writer.WriteLine("{");
    ...
    private static string EmitExpression(CSharpTextWriter writer, MirExpression expression, MirFunction function, IReadOnlySet<string> enumNames, ref int tempIndex, List<CSharpDiagnostic> diagnostics)
        => expression switch
        {
            ...
            MirTableWithExpression withExpression => EmitTableWith(writer, withExpression, function, enumNames, ref tempIndex, diagnostics),
    ...
    private static IEnumerable<MirCallableConstructionExpression> EnumerateCallableConstructions(MirExpression expression)
    {
        ...
        return expression switch
        {
            ...
            MirTableWithExpression update => Children([update.Source, .. update.Replacements.Select(replacement => (MirExpression)replacement.Value)]),
    ...
    private static string EmitTableWith(
        CSharpTextWriter writer,
        ...
        List<CSharpDiagnostic> diagnostics)
    {
        ...
        {
            string replacementValue = EmitExpression(
                writer,
                replacement.Value,
                function,
                enumNames,
                ref tempIndex,
                diagnostics);
            value = $"{value}.{TableWithColumnMethodName(replacement.ColumnId)}({replacementValue})";
        }
        return value;
    }
    ...
    private static bool ProgramUsesArrayElementAccess(MirProgram program)
        => program.Functions.Any(function => function.Body.Any(StatementUsesArrayElementAccess));

    private static bool StatementUsesArrayElementAccess(MirStatement statement)
        => statement switch
        {
            MirVariableDeclarationStatement declaration => ExpressionUsesArrayElementAccess(declaration.Initializer),
            MirResourceUsingDeclarationStatement declaration => ExpressionUsesArrayElementAccess(declaration.Initializer),
            MirExpressionStatement expression => ExpressionUsesArrayElementAccess(expression.Expression),
            MirReturnStatement { Expression: not null } returned => ExpressionUsesArrayElementAccess(returned.Expression),
            MirIfStatement conditional => ExpressionUsesArrayElementAccess(conditional.Condition)
                || conditional.ThenStatements.Any(StatementUsesArrayElementAccess)
                || (conditional.ElseStatements?.Any(StatementUsesArrayElementAccess) ?? false),
            MirWhileStatement loop => ExpressionUsesArrayElementAccess(loop.Condition) || loop.BodyStatements.Any(StatementUsesArrayElementAccess),
            MirForStatement loop => (loop.Initializer is not null && StatementUsesArrayElementAccess(loop.Initializer))
                || (loop.Condition is not null && ExpressionUsesArrayElementAccess(loop.Condition))
                || (loop.Increment is not null && ExpressionUsesArrayElementAccess(loop.Increment))
                || loop.BodyStatements.Any(StatementUsesArrayElementAccess),
            MirForOfStatement loop => ExpressionUsesArrayElementAccess(loop.Iterable) || loop.BodyStatements.Any(StatementUsesArrayElementAccess),
            _ => false,
        };

    private static bool ExpressionUsesArrayElementAccess(MirExpression expression)
        => expression switch
        {
            ...
            MirUnaryExpression unary => ExpressionUsesArrayElementAccess(unary.Operand),
            MirBinaryExpression binary => ExpressionUsesArrayElementAccess(binary.Left) || ExpressionUsesArrayElementAccess(binary.Right),
            MirCallExpression call => call.Arguments.Any(ExpressionUsesArrayElementAccess),
            MirInvokeExpression invoke => ExpressionUsesArrayElementAccess(invoke.Callee) || invoke.Arguments.Any(ExpressionUsesArrayElementAccess),
            MirArrayExpression array => array.Elements.Any(ExpressionUsesArrayElementAccess),
            MirBatchExpression batch => ExpressionUsesArrayElementAccess(batch.Input)
                || batch.Body.PrefixStatements.Any(StatementUsesArrayElementAccess)
                || ExpressionUsesArrayElementAccess(batch.Body.ValueExpression),
            MirRecordConstructionExpression record => record.Initializers.Any(initializer => ExpressionUsesArrayElementAccess(initializer.Value)),
            MirRecordFieldAccessExpression access => ExpressionUsesArrayElementAccess(access.Receiver),
            MirTableWithExpression update => ExpressionUsesArrayElementAccess(update.Source) || update.Replacements.Any(replacement => ExpressionUsesArrayElementAccess(replacement.Value)),
            MirRecordWithExpression update => ExpressionUsesArrayElementAccess(update.Source) || update.Replacements.Any(replacement => ExpressionUsesArrayElementAccess(replacement.Value)),
            MirEnumValueExpression value => value.Arguments.Any(ExpressionUsesArrayElementAccess),
            MirMatchExpression match => ExpressionUsesArrayElementAccess(match.Scrutinee) || match.Arms.Any(arm => ExpressionUsesArrayElementAccess(arm.Expression)),
            MirResultMatchExpression match => ExpressionUsesArrayElementAccess(match.Scrutinee) || ExpressionUsesArrayElementAccess(match.OkExpression) || ExpressionUsesArrayElementAccess(match.ErrExpression),
            MirIfExpression conditional => ExpressionUsesArrayElementAccess(conditional.Condition) || ExpressionUsesArrayElementAccess(conditional.ThenExpression) || ExpressionUsesArrayElementAccess(conditional.ElseExpression),
            _ => false,
        };
    ...
    private static bool StatementUsesTableWith(MirStatement statement)
        => statement switch
        {
    ...
    private static bool ExpressionUsesTableWith(MirExpression expression)
        => expression switch
        {
    ...
    private static bool ExpressionUsesBatch(MirExpression expression)
        => expression switch
        {
            ...
            MirTableWithExpression update => ExpressionUsesBatch(update.Source) || update.Replacements.Any(replacement => ExpressionUsesBatch(replacement.Value)),
    ...
    private static string TableWithColumnMethodName(MirTableColumnId id)
        => "__with_" + EncodeStableIdentity(id.Value);
