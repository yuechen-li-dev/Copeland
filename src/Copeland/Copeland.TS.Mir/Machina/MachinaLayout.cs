...
public static class MachinaBrowserLowerer
{
    ...
    public static MachinaReactArtifact LowerForReact(MachinaResolvedDocument document)
        => LowerForReact(document, artifactNamespace: null);

    /// <summary>
    /// Projects a named layout artifact into React classes. The namespace is
    /// part of the frame-class identity, so independently authored roots can
    /// coexist in one stylesheet without their <c>root</c> frames colliding.
    /// Style classes remain content-addressed and are therefore shared when
    /// their immutable style values are equal.
    /// </summary>
    public static MachinaReactArtifact LowerForReact(MachinaResolvedDocument document, string? artifactNamespace)
    {
        string normalizedNamespace = NormalizeReactArtifactNamespace(artifactNamespace);
        ...
        var nodesByIdentity = document.Nodes.ToDictionary(node => node.Identity, StringComparer.Ordinal);
        var css = new StringBuilder("/* Generated by Copeland Machina. Edit Copeland source, not this CSS. */\n\n");
        ...
        {
            string frameClass = ReactFrameClass(normalizedNamespace, node.Identity);
            ...
            MachinaResolvedNode? parent = node.ParentIdentity is null
                ? null
                : nodesByIdentity[node.ParentIdentity];
            WriteReactFrameRule(node, parent, frameClass, css);
        }

        return new MachinaReactArtifact(css.ToString(), classesByIdentity);
    }

    private static string NormalizeReactArtifactNamespace(string? artifactNamespace)
    {
        ...
        if (artifactNamespace.Any(character => !char.IsAsciiLetterOrDigit(character) && character != '-'))
        {
            throw new MachinaLayoutException(
                "COPE-MACHINA-REACT-0001",
                $"React artifact namespace '{artifactNamespace}' may contain only ASCII letters, digits, and hyphens.");
        }

        return artifactNamespace + "-";
    }

    private static string ReactFrameClass(string artifactNamespace, string identity)
        => "m-frame-" + artifactNamespace + identity.Replace('/', '-');

    private static void WriteReactFrameRule(
