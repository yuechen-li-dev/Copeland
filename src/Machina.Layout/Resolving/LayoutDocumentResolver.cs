...
public static class LayoutDocumentResolver
{
    ...
    private static void ResolveNode(NodeId nodeId, Rect resolvedRect, LayoutDocument document, IDictionary<NodeId, ResolvedLayoutNode> resolvedNodes, IDictionary<NodeId, VisitState> visitState)
    {
        ...
        var childRects = node.Arrange is null ? ResolveDirectChildren(resolvedRect, childIds, document, nodeId) : ResolveArrangedChildren(node.Arrange, resolvedRect, childIds, document, nodeId);
    ...
    private static List<Rect> ResolveDirectChildren(Rect parentRect, IReadOnlyList<NodeId> childIds, LayoutDocument document, NodeId nodeId)
    {
    ...
    {
        if (arrange is not StackArrange stack) throw new LayoutError("UnsupportedArrange", $"Unsupported arrange type '{arrange.GetType().Name}' on node '{nodeId}'.");
        return ResolveStackChildren(stack, parentRect, childIds, document, nodeId);
    }
    ...
    private static List<Rect> ResolveStackChildren(StackArrange stack, Rect parentRect, IReadOnlyList<NodeId> childIds, LayoutDocument document, NodeId nodeId)
    {
        ...
        foreach(var i in infos){var childMain=i.frame is FillFrame ? remaining*(i.weight/totalWeight) : i.fixedMain; var childCross=i.cross; var crossOffset=stack.Align switch{StackAlign.Start=>0,StackAlign.Center=>(crossSize-childCross)/2,StackAlign.End=>crossSize-childCross,_=>0};
    ...
    private static void ValidateFinitePositive(double value,string code,string msg){if(double.IsNaN(value)||double.IsInfinity(value)||value<=0) throw new LayoutError(code,msg);}    private static void ValidateFiniteNonNegative(double value,string code,string msg){if(double.IsNaN(value)||double.IsInfinity(value)||value<0) throw new LayoutError(code,msg);}    
