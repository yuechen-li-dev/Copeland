...
namespace Typography.OpenFont.Tables
{
    class Glyf : TableEntry
    {
        ...
        internal static Glyph CreateEmptyGlyph(ushort glyphIndex)
        {
            return new Glyph(
                Array.Empty<GlyphPointF>(),
                Array.Empty<ushort>(),
                Bounds.Zero,
                null,
                glyphIndex);
        ...
        protected override void ReadContentFrom(BinaryReader reader)
        {
            ...
            for (int i = 0; i < glyphCount; i++)
            {
                ...
                {
                    // Empty-outline glyphs still retain their own glyph identity. Metrics and
                    // shaping clients use GlyphIndex to query hmtx, so aliasing every empty
                    // outline to glyph zero silently gives spaces the .notdef advance width.
                    _glyphs[i] = CreateEmptyGlyph((ushort)i);
                }
            }
        ...
        Glyph ReadCompositeGlyph(Glyph[] createdGlyphs, BinaryReader reader, uint tableOffset, ushort compositeGlyphIndex)
        {
            ...
            return finalGlyph ?? CreateEmptyGlyph(compositeGlyphIndex);
        }
    }
}
