...
internal static class Program
{
    ...
    {
        if (args.Length != 3 || (args[0] != "loopback" && args[0] != "scenario" && args[0] != "session-bootstrap" && args[0] != "eligible-host-fixtures") || args[1] != "--config")
        {
            Console.Error.WriteLine("Usage: Aurelian.Marionette.Transport <loopback|scenario|session-bootstrap|eligible-host-fixtures> --config <ignored-local-config.json>");
            return 2;
        }
