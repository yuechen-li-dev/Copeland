...
public static class OblivionMarkdownRenderer
{
        ...
        double height)
    {
        ...
        {
            return BuildPlainPreviewBody(id, projection.Preview, theme, width, height);
        }
    ...
    public static double MeasureExpandedContentHeight(OblivionCardBody body, double width)
    {
        ...
        if (projection.Document is null)
        {
            IReadOnlyList<string> lines = projection.Preview.Count == 0 ? ["<empty>"] : projection.Preview;
            return MeasureWrappedPlainTextHeight(lines, width);
        }

        return MeasureMarkdownDocumentHeight(projection.Document, width);
    }
        ...
        double requestedScrollOffset)
    {
        ...
        {
            double currentTop = -scrollOffset;
            foreach ((DocumentBlockMir block, int index) in projection.Document.Blocks.Select((value, index) => (value, index)))
            {
    ...
    public static IReadOnlyList<string> BuildPreviewLines(DocumentMir mir, IReadOnlyList<OblivionCardDiagnostic> diagnostics)
    {
    ...
    public static IReadOnlyList<string> BuildInspectorLines(OblivionCardBody body)
    {
        ...
        if (projection.Document is null)
        {
            return projection.Preview.Count == 0 ? ["<empty>"] : projection.Preview;
        }
    ...
    public static IReadOnlyList<string> BuildDiagnosticLines(IReadOnlyList<OblivionCardDiagnostic> diagnostics)
    {
        ArgumentNullException.ThrowIfNull(diagnostics);
    ...
    public static int GetRawMarkdownSourceLayoutBuildCountForBody(OblivionCardBody body)
    {
        ...
        string sourceIdentity = body.RawText;
        ...
        return RawMarkdownSourceLinesCache.GetOrAdd(
            ...
            static rawText => BuildRawMarkdownSourceLines(
                new OblivionCardBody(
                    ...
                    new OblivionInlineMarkdownContent(rawText))));
    }
    ...
    private static string FormatDiagnosticLine(OblivionCardDiagnostic diagnostic)
    {
