...
using static Oblivion.Product.OblivionMarkdownBody;

namespace Oblivion.App;

public static class OblivionWorkbench
{
    ...
    public static IReadOnlyList<OblivionCard> CreateAllCards(OblivionHostOptions? proofOptions = null)
    {
        return
        [
    ...
    public static IReadOnlyList<UiRow> BuildPageRows(
        ...
        OblivionHostState? navigationState = null,
        OblivionShellMode shellMode = OblivionShellMode.Wide)
    {
        ArgumentNullException.ThrowIfNull(theme);

        OblivionEffectState effectState = navigationState?.EffectState ?? OblivionEffectState.Empty;
        ...
        bool compactInspector = shellMode == OblivionShellMode.Compact &&
            navigationState?.CompactPane == OblivionCompactPane.Inspector;
        OblivionPageLayout layout = shellMode == OblivionShellMode.Compact
            ? OblivionPageLayout.CreateCompact(contentWidth, viewportHeight)
            : OblivionPageLayout.CreateWide(contentWidth, viewportHeight);

        if (shellMode == OblivionShellMode.Compact)
        {
        ...
        OblivionShellMode shellMode = OblivionShellMode.Wide)
    {
        ...
        IReadOnlyList<OblivionBuiltCard> cards = GetBuiltPageCards(pageId, proofOptions, OblivionEffectState.Empty, navigationState);
    ...
    {
        return CardHandlers.BuildCard(card, effectState: OblivionEffectState.Empty).CompactView.PreferredHeight;
    }
        ...
        OblivionHostOptions? proofOptions = null)
    {
        ...
        string workspacePath = OblivionWorkspacePaths.ResolveWorkspaceManifestPath(proofOptions?.WorkspacePath);
        ...
        OblivionWorkspaceLoadResult loadResult = usingFallbackCatalog
            ? new OblivionWorkspaceLoadResult(null, null, [])
    ...
    public static IReadOnlyList<OblivionBuiltCard> GetBuiltPageCardsForSelection(
        ...
        OblivionHostState? navigationState = null)
    {
        return GetBuiltPageCards(pageId, proofOptions, effectState ?? OblivionEffectState.Empty, navigationState);
    }
        ...
        OblivionHostOptions? proofOptions = null)
    {
        ...
        OblivionCard? alias = cards.FirstOrDefault(card =>
            string.Equals(card.Provenance.SourceReference is null ? null : Path.GetFileNameWithoutExtension(Path.GetFileNameWithoutExtension(card.Provenance.SourceReference)), normalized, StringComparison.OrdinalIgnoreCase) ||
            string.Equals(card.Provenance.SourceReference is null ? null : Path.GetFileNameWithoutExtension(card.Provenance.SourceReference), normalized, StringComparison.OrdinalIgnoreCase));
        ...
        OblivionShellMode shellMode)
    {
        IReadOnlyList<OblivionBuiltCard> cards = GetBuiltPageCards(
            pageId,
            proofOptions,
            navigationState?.EffectState ?? OblivionEffectState.Empty,
            navigationState);
        ...
        if (shellMode == OblivionShellMode.Wide)
        {
            ...
            scrollRegions.Add(new OblivionScrollRegionTarget(
                new OblivionScrollTarget(OblivionScrollTargetKind.MainCardStack, pageId),
                cardsPaneBounds,
                cardsPaneScrollbar,
                cardsPaneContentHeight));
            ...
            ScrollbarGeometry inspectorPaneScrollbar = ScrollRegion.ComputeScrollbarGeometry(
            ...
            scrollRegions.Add(new OblivionScrollRegionTarget(
                new OblivionScrollTarget(OblivionScrollTargetKind.InspectorPane, pageId),
                inspectorPaneBounds,
        ...
        foreach (OblivionBuiltCard builtCard in cards)
        {
            ...
            CardFrame frame = OblivionCardRenderer.DescribeFrame(resolved, cardId);
            Rect headerBounds = OblivionCardRenderer.DescribeHeaderHitRect(resolved, cardId);
            UiActionId toggleAction = OblivionUiActions.ToggleCardExpansion(pageId, cardId);
            ...
            if (viewport is not null)
            {
                bodyTargets.Add(
                    new OblivionCardBodyHitTarget(
                        ...
                        cardId,
                        viewport.Bounds,
                        OblivionUiActions.SelectCard(pageId, cardId),
                        viewport.ScrollbarGeometry,
                        viewport.ContentHeight));
                scrollRegions.Add(
                    new OblivionScrollRegionTarget(
                        new OblivionScrollTarget(OblivionScrollTargetKind.ExpandedMarkdownBody, pageId, cardId),
                        viewport.Bounds,
                ...
                TryFindRectBySuffix(resolved, $"{pageId}.{cardId.Replace(".", "-", StringComparison.Ordinal)}{WideInspectorRawSourceViewportBaseSuffix}.source-frame", out Rect rawSourceBounds))
            {
                ...
                ScrollbarGeometry rawSourceScrollbar = ScrollRegion.ComputeScrollbarGeometry(
                ...
                scrollRegions.Add(
                    new OblivionScrollRegionTarget(
                        new OblivionScrollTarget(OblivionScrollTargetKind.InspectorRawMarkdownSource, pageId, cardId),
                        rawSourceBounds,
                        rawSourceScrollbar,
                        rawSourceContentHeight));
            }
        ...
        OblivionHostLayout layout)
    {
        ...
        CardLayout cardLayout = OblivionCardRenderer.ComputeLayout(
            builtCard.CompactView,
            renderOptions,
        ...
        return ScrollRegion.ClampScrollOffset(contentHeight, viewportHeight, requestedOffset);
    }
    ...
    public static OblivionEffectOutcome? InvokeCardAction(
        string pageId,
        string cardId,
        string actionId,
        OblivionHostOptions? proofOptions = null,
        OblivionEffectState? effectState = null)
    {
        ...
        var application = new OblivionApplication(CardHandlers);
        OblivionActionOutcome? outcome = application.Invoke(
            card,
            pageId,
            actionId,
            new OblivionApplicationState(effectState ?? OblivionEffectState.Empty));
        return outcome is null
            ? null
            : new OblivionEffectOutcome(outcome.Request, outcome.Result);
    }
    ...
    private static IReadOnlyList<OblivionCard> GetPageCards(string pageId, OblivionHostOptions? proofOptions)
    {
        ...
        OblivionWorkspacePage? page = loadResult.Workspace.Sections
            .SelectMany(section => section.Pages.Select(candidate => new
            {
                HostPageId = $"{section.Id}.{candidate.Id.Value}",
                Page = candidate,
            }))
            .FirstOrDefault(candidate => string.Equals(candidate.HostPageId, pageId, StringComparison.Ordinal))?
            .Page;
        ...
        if (page is null)
        {
            ...
            [
                CreateStatusCard(
                    ...
                    [$"Requested page '{pageId}' was not found in '{loadResult.Location?.ManifestPath ?? "<unknown>"}'."],
                    [])
            ];
        }
        ...
        return page.Cards
            ...
            {
                PageId = new OblivionPageId(pageId),
                WorkspaceId = loadResult.Workspace.Id,
            })
            .ToArray();
    }
        ...
        OblivionPageLayout layout)
    {
        ...
        if (selection.SelectedCard is null)
        {
            rows.Add(Row.Anchor(
                ...
                width: layout.InspectorWidth,
                height: 180,
                component: StandardCard.BuildTextCard(
                    ...
                    theme: theme,
                    options: new StandardCardOptions(layout.InspectorWidth, 180))));
            return rows;
        }
        ...
        if (selection.SelectedCardId is not null)
        {
            rows.Add(Row.Anchor(
                ...
                component: StandardUI.Button(
                    ...
                    action: OblivionUiActions.SetCompactPane(OblivionCompactPane.Inspector).ToAction(),
                    theme: theme,
        ...
        [
            Row.Anchor(
                ...
                component: StandardUI.Button(
                    ...
                    action: OblivionUiActions.SetCompactPane(OblivionCompactPane.CardList).ToAction(),
                    theme: theme,
                    variant: ButtonVariant.Outline,
                    size: ButtonSize.Medium)),
            Row.Anchor(
            ...
            rows.Add(
                Row.Anchor(
                    ...
                    component: StandardCard.BuildTextCard(
                        ...
                        theme: theme,
                        options: new StandardCardOptions(layout.ContentWidth, 180))));
            return rows;
        }
            ...
            children.Add(
                UI.Anchor(
                    StandardCard.BuildTextCard(
                        ...
                        theme: theme,
                        options: new StandardCardOptions(contentWidth, 180)),
        ...
        double rawMarkdownSourceScrollOffset = 0)
    {
        ...
        if (section.Body is OblivionInspectorRawMarkdownSourceBodyContent rawMarkdownBody)
        {
            ...
            return StandardCard.BuildHostedCard(
                ...
                body: rawSource.Node,
                theme: theme,
                options: new StandardCardOptions(layout.InspectorWidth, section.Height, ClipContent: section.ClipContent));
        }
        ...
        OblivionHostOptions? proofOptions = null)
    {
        ...
        IReadOnlyList<OblivionBuiltCard> builtCards = cards
            .Select(card => CardHandlers.BuildCard(card, card.PageId?.Value, card.WorkspaceId?.Value, navigationState.EffectState))
        ...
        OblivionHostOptions? proofOptions = null)
    {
        ...
        int diagnosticsCount = markdownCards.Sum(card => OblivionMarkdownBody.Project(card.Body).Diagnostics.Count);
        string[] markdownCardIds = markdownCards.Select(card => card.Id.Value).ToArray();
        string[] markdownBodySourcePaths = markdownCards
            .Select(card => card.Body.SourceReference ?? "<inline>")
            .Distinct(StringComparer.Ordinal)
        ...
        OblivionHostOptions? proofOptions = null)
    {
        ...
        IReadOnlyList<OblivionCard> cards = CreateAllCards(proofOptions)
            ...
            .ToArray();
        IReadOnlyList<OblivionBuiltCard> builtCards = cards
            .Select(card => CardHandlers.BuildCard(card, card.PageId?.Value, card.WorkspaceId?.Value, OblivionEffectState.Empty))
            .OrderBy(card => card.SourceCard.Id.Value, StringComparer.Ordinal)
            .ToArray();
